...
@section('container')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Edit Cuti Karyawan "{{ $karyawan->nama }}"</h1>
    </div>

    @if (session()->has('success'))
        <div class="alert alert-success alert-dismissible fade show" role="Alret">
            {{ session('success') }}
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="/cutikaryawan/{{ $karyawan->id }}" style="text-decoration: none">Cuti
                        Karyawan</a></li>
                <li class="breadcrumb-item active" aria-current="page">Edit Cuti Karyawan</li>
            </ol>
        </nav>
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <form action="/cutikaryawan/{{ $cutiKaryawan->id }}" method="post">
                    @method('PUT')
                    @csrf
                    <div class="row">
                    ...
                    <div class="mb-3 col-lg-6">
                        <input type="text" name="karyawan_id" id="karyawan_id" class="form-control"
                            value="{{ $karyawan->id }}" readonly hidden>
                    </div>
                    <hr>
                    <h5>Edit Cuti Karyawan</h5>
                    @if ($jatahCuti <= 0)
                    ...
                    <div class="mb-3 col-lg-4">
                        ...
                        <select name="cuti" id="cuti" class="form-select">
                            @foreach ($cutis as $cuti)
                                @if (old('cuti', $cutiKaryawan->cuti) == $cuti->cuti)
                                    <option value="{{ $cuti->cuti }}" selected>{{ $cuti->cuti }}, {{ $cuti->hari }}
                                        hari</option>
                                @else
                                    <option value="{{ $cuti->cuti }}">{{ $cuti->cuti }}, {{ $cuti->hari }}</option>
                                @endif
                            @endforeach
                        </select>
                    </div>
                    <div class="row">
                        <div class="mb-3 col-lg-4">
                            ...
                            <input type="date" name="tanggal_cuti" id="tanggal_cuti"
                                class="form-control @error('tanggal_cuti') is-invalid @enderror" placeholder="dd/mm/yyyy"
                        ...
                        <div class="mb-3 col-lg-4">
                            ...
                            <p class="form-control">{{ $cutiKaryawan->tanggal_cuti }}</p>
                        </div>
                        <div class="mb-3 col-lg-4">
                            ...
                            <input type="number" class="form-control" name="jumlah_hari" id="jumlah_hari"
                                value="{{ $cutiKaryawan->jumlah_hari }}" readonly>
                        </div>
                    </div>
                    @if ($jatahCuti >= 1)
